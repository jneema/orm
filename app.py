...
pw_hash = flask_bcrypt.generate_password_hash(
    'form.user.password').decode('utf-8')
...
class Sales(db.Model):
    ...
    product = db.relationship(
        'Products', backref=db.backref('sales', lazy=True))
...
def sales(x):
    ...
    # .all()
    r = db.session.query(Products.name, db.func.sum(Sales.quantity).label("Quantity"), db.func.sum((Products.sp-Products.bp) *
                         Sales.quantity).label("Profit")).join(Sales, Products.id == Sales.product_id).group_by(Products.name).filter(Products.id == x).all()
...
def total_sales():
    ...
    for result in r:
        print(' Name:', result[0], 'Quantity:',
              result[1], 'Profit:', result[2])
    # # print(list_sales)
    # sales = Sales.query.all()
    return render_template('sales.html', sales=r)
...
def makesale():
    ...
    row = Sales(product_id=product_id,
...
def dashboard():
    ...
    spm = Sales.query.with_entities(func.sum(Sales.quantity * Products.sp), extract(
        'month', Sales.created_at)).join(Products).group_by(extract('month', Sales.created_at))
    ...
    sbp = Sales.query.with_entities(func.sum(Sales.quantity * Products.sp), Products.name).join(
        Products).group_by(Products.name).order_by(func.sum(Sales.quantity * Products.sp))
...
#   cur.execute("SELECT sum(s.quantity * p.sp) AS sales,DATE(created_at) AS today FROM sales s JOIN products p ON p.id = s.product_id WHERE DATE(created_at) = current_date GROUP BY today;")
    ds = Sales.query.with_entities(func.sum(Sales.quantity * Products.sp), func.date(Sales.created_at)).join(
        Products).filter(func.date(Sales.created_at) == date.today()).group_by(func.date(Sales.created_at))
...
#   monthly sales
    ms = Sales.query.with_entities(func.sum(Sales.quantity * Products.sp), extract('month', Sales.created_at)).join(
        Products).filter(extract('month', Sales.created_at) == date.today().month).group_by(extract('month', Sales.created_at))
    monthsales = 0
    thismonth = []
    for i in ms:
...
#   cur.execute('SELECT p.name,sum(s.quantity) AS most_sold FROM sales s JOIN products p ON p.id = s.product_id GROUP BY p.name ORDER BY (sum(s.quantity)) DESC LIMIT 5;')
    prod = []
    data5 = []
    top5 = Sales.query.with_entities(Products.name, func.sum(Sales.quantity)).join(
        Products).group_by(Products.name).order_by(func.sum(Sales.quantity)).limit(5)
